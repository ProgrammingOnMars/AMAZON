...
def extract(html):
    ...
    element = select.xpath('//*[@id="searchDropdownBox"]//option/text()')
    print(element)

    for ele in element:
        print(ele)
        code = uuid.uuid1()
        sql = 'insert into classification (code, name) values ("{}", "{}")'.format(code, ele)
        write_sql(sql)
...
extract(request_inter_function(url))
